...
//                }
            ...
            //根据具体的admin_id与角色id映射查询出角色名称
            foreach ($rs['data'] as $k => $v) {
                $res['data'][] = Tools::object2array($v);
            }
            foreach ($res['data'] as $k => $v) {
                $ar_res = AuthServer::getAdminRole(['admin_id' => $v['id']]);
                if ($ar_res['code'] == 0) {
                    $ah_res = AuthServer::getAuthRole(['id' => $ar_res['data'][0]['role_id']]);
                    if ($ah_res['code'] == 0) {
                        $res['data'][$k]['role_name'] = $ah_res['data'][0]['role_name'];
                    } else {
                        $res['data'][$k]['role_name'] = '';
                    }
                } else {
                    $res['data'][$k]['role_name'] = '';
                }
            }
            $this->out('0', '查询成功', ['data' => $res['data'], 'c_count' => $c_count, 'count' => $count, 'pages' => $pages]);
//            }else{
