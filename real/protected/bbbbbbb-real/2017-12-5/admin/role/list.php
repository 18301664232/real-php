        ...
        <div class="search-box">
            <div class="row">
                ...
                <div class="input-group col-sm-4">
                    <input type="text" class="form-control search-key-input" placeholder="角色名<?php echo $this->id .'/add' ?>">
...
<script>
    ...
    function checkAuth() {
        //执行权限--以下是在没有权限的情况下操作
        if (<?php echo $this->checkAuth('admin/auth/add') ?>) {
            $('.add-ok').attr('disabled', 'disabled');

        }
        if (<?php echo $this->checkAuth('admin/auth/list') ?>) {
            $('.search-key').attr('disabled', 'disabled');
        }
        if (<?php echo $this->checkAuth('admin/auth/edit') ?>) {
