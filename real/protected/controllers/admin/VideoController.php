...
class VideoController extends CenterController
{
    ...
    public function actionProDetial()
    {
        ...
        $rs['data']['show_url'] = REAL . U('product/index/index') . '&url_type=pc&id=' . $rs['data']['uid'];
    ...
    public function actionForceOut()
    {
        ...
        $params['product_name'] = !empty($_REQUEST['product_name']) ? $_REQUEST['product_name'] : '';
...
//                break;
        ...
        try {
            ...
            if (!empty($user_tel)) {
                if ($params['type'] == 'p_no') {
                    $rs = CommonInterface::sendAliyunMsg($user_tel, array('productname' => $params['product_name'], 'productid' => $params['product_id'], 'productreason' => $params['reason']), 'SMS_113455629');
                }
            }
            $transaction->commit();
        } catch (Exception $e) {
