...
<script type="text/javascript">
    ...
    var logonUserNameIsOk = false;//邮箱是否可用
...
//			$(this).parent().find('p').html('请输入邮箱地址');
        } else if (!($(this).val().match(logonEmailReg))) {
            $(this).parent().addClass('logonResponse');
            $(this).parent().find('p').html('邮箱格式不正确');
        } else {//判断邮箱是否存在
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.code == 0) {
                        ...
                        $('.logonUserName > input').parent().find('p').html('邮箱已存在');
                    }
                }
            });
        }
    ...
    $('.logonSubmit > button').on('click', function () {
        ...
        if ($('.logonAgreement').prop('checked')) {
            if (logonClickDoor) {
                ...
                logonUserNameIsOk = false;
                logonVerifySecIsOk = false;
                if (logonUserNameStr === '') {//判断邮箱是否正确
                    $('.logonUserName').addClass('logonResponse');
                    $('.logonUserName > p').html('请输入邮箱地址');
                } else if (!(logonUserNameStr.match(logonEmailReg))) {
                    $('.logonUserName').addClass('logonResponse');
                    $('.logonUserName > p').html('邮箱格式不正确');
                } else {
                    url = '<?php echo U('user/register/usernameisok') ?>';
                    $.ajax({
                        type: "POST",
                ...
                if (($('.logonUserName > input').val() !== '') && ($('.logonUserName > input').val().match(logonEmailReg)) && ($('.logonVerifySec > input').val() !== '') && ($('.logonUserPassWord > input').val() !== '') && ($('.logonUserPassWord > input').val().length >= 6) && ($('.logonUserPassWord > input').val().match(logonPassWordReg))) {
                    ...
                    var is_read = 1;
                    var type = 2;
                    url = '<?php echo U('user/register/indexs') ?>';
                    $.ajax({
                        type: "POST",
                        url: url,
                        dataType: "json",
